...
from bestee_compute.etf.holdings import _resolve_provider
...
def normalize_holdings(df: pl.DataFrame, issuer: str) -> pl.DataFrame:
    ...
    Args:
        df: The provider's native holdings DataFrame, as returned by
            :func:`bestee_compute.etf.<provider>.get_holdings`.
...
def get_holdings_normalized(ticker: str) -> pl.DataFrame:
    ...
    Raises:
        bestee_compute.etf.IssuerNotSupportedError: If *ticker*'s issuer has no
            registered scraper.
    """
