"""Tests for bestee_compute.stocks.decorators — KnowledgeBase pipeline."""
...
_GET_TS_PATCH = "bestee_compute.stocks.decorators.get_time_series"
_FINANCIALS_PATCH = "bestee_compute.stocks.decorators.build_financials_df"
...
_STRESS_INPUTS = sorted(
    p.name
    for p in importlib.resources.files(bestee_compute.resources).iterdir()
...
def _read_stress(filename: str) -> list[list[str]]:
    text = (
        importlib.resources.files(bestee_compute.resources)
        .joinpath(filename)
...
def test_full_pipeline_executes_end_to_end_with_mocks() -> None:
    ...
    with (
        patch(
            "bestee_compute.stocks.decorators.AssetScopeDecorator.build_kb",
...
class TestIndicatorAliasChaining:
    ...
    def test_chained_indicators_compute_end_to_end(self, mock_get: MagicMock) -> None:
        ...
        """
        from bestee_compute.stocks.builder import _process_rsi, _process_sma
