...
Each task also accepts the requester's ``user_email`` (extracted from the Clerk
session token by the API, ``None`` when the token doesn't carry it), logged for
attribution and available for any per-user follow-up.
...
def optimize_clustering(
    request: dict[str, Any], user_email: str | None = None
) -> dict[str, str]:
    ...
    logger.info("clustering %s requested by %s", rid, user_email or "anonymous")
...
def optimize_regime(
    ...
    result.save(_RESULTS_DIR / rid)
    logger.info("regime %s requested by %s", rid, user_email or "anonymous")
    return {"result_id": rid}
...
def optimize_fama_french(
    ...
    result.save(_RESULTS_DIR / rid)
    logger.info("fama_french %s requested by %s", rid, user_email or "anonymous")
    return {"result_id": rid}
...
def optimize_rrg(
    ...
    result.save(_RESULTS_DIR / rid)
    logger.info("rrg %s requested by %s", rid, user_email or "anonymous")
    return {"result_id": rid}
